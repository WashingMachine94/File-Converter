...
closeButton.addEventListener("click", function () { ipcRenderer.send('app/close'); });
minimizeButton.addEventListener("click", function () { ipcRenderer.send('app/minimize'); });

const SettingsIcon = document.getElementById("SettingsIcon");
document.getElementById("SettingsButton").addEventListener("mouseover", function () { SettingsIcon.classList.add('SettingsIconHover') });
document.getElementById("SettingsButton").addEventListener("mouseout", function () { SettingsIcon.classList.remove('SettingsIconHover') });
...
droparea.addEventListener("drop", (e) => {
    ...
    for (var i = 0; i < e.dataTransfer.files.length; i++) { PushFileInfo(e.dataTransfer.files[i]); }
    droparea.classList.remove("hover");
});

// Clear before uploading files so the onchange can detect the files being uploaded.
directoryUpload.onclick = function () { this.value = null; }
fileUpload.onclick = function () { this.value = null; }

// from files upload button
function UploadFiles(files) { for (var i = 0; i < files.length; i++) { PushFileInfo(files[i]); } }

// from directory upload butotn
function UploadDirectory() {
    var input = document.getElementById("directory-upload");
    for (var i = 0; i < input.files.length; i++) { PushFileInfo(input.files[i]); }
}
...
function HandleFile(fileObject) {
    ...
    for(var i in ConvertTypes["ConvertGroups"]) {
        ...
        if(!JSON.stringify(convertGroup).includes('"' + fileObject.filetype + '"')) 
            continue;

        // create convertgroup if it doesn't exist.
        var convertGroupElement = document.getElementById('ConvertGroupDiv.' + convertGroupName);

        if(convertGroupElement == undefined || convertGroupElement == null) {
            CreateConvertGroup(convertGroup, convertGroupName, fileListContainer);
            convertGroupElement = document.getElementById('ConvertGroupDiv.' + convertGroupName);
        }

        // create filetypeElement if it doesn't exist
        var fileTypeElement = document.getElementById("FileTypeDiv." + fileObject.filetype);
        if(fileTypeElement == undefined) {
            CreateFileType(convertGroup, convertGroupName, fileObject.filetype, convertGroupElement);
            fileTypeElement = document.getElementById("FileTypeDiv." + fileObject.filetype);
        }
...
function CreateConvertGroup(convertGroupObject, convertGroupName, parent) {
    ...
    convertButton.addEventListener('click', (e) => {
        const convertType = e.target.parentElement.parentElement.id.split(".")[1];
...
function CreateFileType(convertGroupObject, convertGroupName, fileType, parent) {
    ...
    fileRemoveButton.addEventListener('click', (event)=> { 
        if(event.target.parentElement.parentElement.parentElement.children.length < 3) {
...
function TranslateConvertType(convertType) {
    const convertTypeElement = document.getElementById("ConvertGroupDiv." + convertType);
    ...
    for(var i=0; i < convertTypeElement.children.length; i++) {
        ...
        for(const file of fileList.children) {
            ...
            if(fileType == finalfiletype) 
                continue;

            convertHandler(filepath, exportPath, finalfiletype, convertType).then((result) => {
                console.log(result);
                removeFile(file);
            }).catch((error) => {
...
function TranslateFileType(fileType, convertType) {
    ...
    var FileTypeElement = document.getElementById("FileTypeElement." + fileType);
    var convertDropdown = FileTypeElement.children[0].children[1];
    var finalfiletype = convertDropdown.value;

    for(const file of FileTypeElement.children[1].children) {
        var filepath = file.getAttribute("path");
        var fileDirectory = filepath.substring(0, filepath.lastIndexOf("\\")+1);
        var filename = file.getAttribute("filename").split('.')[0];
        var exportPath = `${fileDirectory}${filename}.${finalfiletype}`;
